if __name__ == '__main__':
    ...
    for line in lines:
        card, bid = line.split(' ')
        cards.append(card.replace('A', 'Z').replace('K', 'Y').replace('Q', 'X').replace('J', '0'))
        bids.append(int(bid))
        if len(set(card)) == 1:  # Five of a kind
            type_arr.append(7)
        elif len(set(card)) == 2:
            if 'J' in card:
                type_arr.append(7)  # Five of a kind JOKER
            elif any([True for c in card if card.count(c) == 4]):
                type_arr.append(6)  # Four of a kind
            else:
                type_arr.append(5)  # Full House
        elif len(set(card)) == 3:
            if card.count('J') >= 2:
                type_arr.append(6)  # Four of a kind JOKER
            elif card.count('J') == 1:
                if any([True for c in card if card.count(c) == 3]):
                    type_arr.append(6)
                else:
                    type_arr.append(5)
            elif any([True for c in card if card.count(c) == 3]):
                type_arr.append(4)  # three of a kind
            else:
                type_arr.append(3)  # two pair
        elif len(set(card)) == 4:
            if 'J' in card:
                type_arr.append(4)  # three of a kind JOKER
            else:
    ...
    sorted_total = [[card, hand] for card, _, hand in sorted(zip(cards, bids, type_arr), key=lambda pair: (pair[2], pair[0]))]
    total_sum = 0
